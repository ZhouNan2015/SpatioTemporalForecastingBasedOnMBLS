...
sunset = max(ts(idx));
% test set contains data of 20 days
n = sunset - sunrise + 1;  Ntest = n * 10;
Nlead = 12;

%%%%%%% Generate dataset of sunny days %%%%%%
% validM: index of meteorological data for PV forecasting (1. wind speed;
% 2. temperature; 3. solar irradiance)

% NlagP: number of measured historical power output used as input factors 
% of PV forecasting

% NlagM: number of measured historical meteorological data used as input  
% factors of PV forecasting
validMSun = [3];  NlagPSun = 18; NlagMSun = 1;
...
validMNon = [3];  NlagPNon = 23; NlagMNon = 2;
...
% Save the dataset as .mat files for PV power forecasting
filename1 = "dataset" + num2str(PVNum) + SEASON + "Sun.mat";
filename2 = "dataset" + num2str(PVNum) + SEASON + "Non.mat";
save(filename1, 'trainXSun', 'trainYSun', 'testXSun', 'testYSun', 'sunset', 'sunrise')
save(filename2, 'trainXNon', 'trainYNon', 'testXNon', 'testYNon', 'sunset', 'sunrise')
